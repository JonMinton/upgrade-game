...
// Crumbled masonry: broken brick courses.
PATTERNS[Tl.RUIN] = [0xdb, 0x89, 0x00, 0x9d, 0x94, 0x00, 0x6b, 0x4a];
// Pushstone: a faceted boulder — diamond silhouette so it reads as "special"
// even at tiers where colour can't carry the distinction.
PATTERNS[Tl.PUSH] = [0x18, 0x3c, 0x7e, 0xff, 0xff, 0x7e, 0x3c, 0x18];
// Burnt stump: a low jagged stub where a tree used to be.
PATTERNS[Tl.STUMP] = [0x00, 0x00, 0x00, 0x54, 0x7c, 0x38, 0x38, 0x7c];
// Cracked stone: the standing-stone silhouette with a fissure zigzagging down.
PATTERNS[Tl.CRACK] = [0x18, 0x34, 0x2c, 0x34, 0x2c, 0x34, 0x6e, 0x7a];
// Split boulder: the rock silhouette with a fissure zigzagging through it.
PATTERNS[Tl.CLEFT] = [0x00, 0x34, 0x6e, 0xf7, 0xef, 0x76, 0x2c, 0x00];
// Tree on fire: two flame frames swapped by the renderer for flicker.
export const BURN_TILE = [
  [0x10, 0x54, 0x7c, 0xfe, 0xfe, 0x7c, 0x18, 0x18],
  [0x44, 0x2a, 0x7c, 0xee, 0xfe, 0x7c, 0x18, 0x18],
];
